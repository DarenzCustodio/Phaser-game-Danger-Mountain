...
export default class Level extends Phaser.Scene {
  ...
  editorCreate(): void {
    // runRight
    const runRight = this.input.keyboard!.addKey(
    ...
    // runLeft
    const runLeft = this.input.keyboard!.addKey(
    ...
    // e_runRightKey
    const e_runRightKey = this.input.keyboard!.addKey(
      Phaser.Input.Keyboard.KeyCodes.RIGHT
    );

    // e_jumpKey
    const e_jumpKey = this.input.keyboard!.addKey(
      Phaser.Input.Keyboard.KeyCodes.UP
    );

    // e_runLeftKey
    const e_runLeftKey = this.input.keyboard!.addKey(
      Phaser.Input.Keyboard.KeyCodes.LEFT
    );

    // background_glacial_mountains
    const background_glacial_mountains = this.add.image(
      1725,
      710,
      "background_glacial_mountains"
    );
    ...
    const snowPlatformLeftPrefab_2 = new SnowPlatformLeftPrefab(
      this,
      1505,
      925
    );
    ...
    const snowPlatformLeftPrefab_3 = new SnowPlatformLeftPrefab(
      this,
      2000,
      685
    );
    ...
    const snowPlatformLeftPrefab_4 = new SnowPlatformLeftPrefab(
      this,
      2525,
      915
    );
    ...
    const swordsman_snowGround_collider = this.physics.add.collider(
      swordsman,
      snowGroundLayer.list
    );
    ...
    const coins_snowGround_collider = this.physics.add.collider(
      coinsLayer.list,
      snowGroundLayer.list
    );

    // coins_snowPlatforms_collider
    const coins_snowPlatforms_collider = this.physics.add.collider(
      coinsLayer.list,
      snowPlatformLayer.list
    );

    // swordsman_coins_collider
    const swordsman_coins_collider = this.physics.add.overlap(
      swordsman,
      coinsLayer.list,
      this.collectCoins as any,
      undefined,
      this
    );

    // snowballs_snowGround_collider
    const snowballs_snowGround_collider = this.physics.add.collider(
      snowballsLayer.list,
      snowGroundLayer.list
    );

    // snowballs_snowPlatforms_collider
    const snowballs_snowPlatforms_collider = this.physics.add.collider(
      snowballsLayer.list,
      snowPlatformLayer.list
    );

    // swordsman_snowballs_collider
    const swordsman_snowballs_collider = this.physics.add.collider(
      swordsman,
      snowballsLayer.list,
      this.hitSnowball as any,
      undefined,
      this
    );

    // enchantress_snowGround_collider
    const enchantress_snowGround_collider = this.physics.add.collider(
      enchantress,
      snowGroundLayer.list
    );
    ...
    const enchantress_coins_collider = this.physics.add.overlap(
      enchantress,
      coinsLayer.list,
      this.enchantressCollectCoins as any,
      undefined,
      this
    );

    // enchantress_snowballs_collider
    const enchantress_snowballs_collider = this.physics.add.collider(
      enchantress,
      snowballsLayer.list,
      this.enchantressHitSnowball as any,
      undefined,
      this
    );
  ...
  private snowPlatforms!: Array<
    SnowPlatformRightPrefab | SnowPlatformMiddlePrefab
  >;
  ...
  private enchantressHitSnowball(
    enchantress: EnchantressPrefab,
  ...
  update() {
    ...
    this.MenuInstructions = this.add.text(
      cameraScrollX + 1150,
      cameraScrollY - offsetY,
      "Press [P] to Pause or [Q] to Quit",
      {
